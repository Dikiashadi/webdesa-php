...
$pdf->AddPage();
  ...
  $pdf->SetFont('Arial','B','16');
      $pdf->Cell(270,7,'Laporan Penduduk Berdasarkan '.$filter.' / '.$pencarian,0,1,'C');
...
$kasus=mysqli_query($koneksi,"SELECT * FROM tb_permohonanktp as k JOIN tb_penduduk as p ON k.nik=p.nik WHERE tempatLahir LIKE '%$pencarian%' ORDER BY no_permohonanKtp ASC") or die (mysqli_error($koneksi)) ;
...
$pdf->Cell(10,8,' :',0);
$pdf->MultiCell(50,8,$row['alamat']." RT.".$row['rt']." RW.".$row['rw']." Kelurahan ".$row['kelurahan']." Kecamatan Bumigora Kota Bumigora",0,1);
